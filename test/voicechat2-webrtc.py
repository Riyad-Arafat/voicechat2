    ...
    try:
        while True:
            ...
            if data["type"] == "offer":
                ...
                if (
                    conversation_manager.sessions[session_id]["peer_connection"]
                    is not None
                ):
                        ...
                        try:
                            ...
                            if line_text.startswith("data: "):
                                ...
                                if "choices" in data and len(data["choices"]) > 0:
                                    ...
                                    if content:
                                        await process_llm_content(
                                            channel, session_id, content
                                        )
...
async def process_llm_content(channel, session_id, content):
    ...
    for sentence in sentences:
        if sentence:
            ...
            await channel.send(
                json.dumps({"type": "llm_output", "content": processed_sentence})
            )
...
async def generate_and_stream_tts(channel, session_id):
    llm_output_sentences = conversation_manager.sessions[session_id][
        "llm_output_sentences"
    ]
    ...
    while llm_output_sentences:
        ...
        if sentence:
            ...
            await channel.send(
                json.dumps({"type": "tts_complete", "sentence": sentence})
            )
